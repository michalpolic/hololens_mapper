...
from src_utils import folders_extensions

class ConvertImages():

    def write_bytes_to_png(self,bytes_path, width, height):
    ...
    def get_width_and_height(self,path):
        with open(path) as f:
            lines = f.readlines()
    ...
    def convert_images(self,folder):
        # p = multiprocessing.Pool(multiprocessing.cpu_count())
        for (img_folder, extension) in folders_extensions:
            if img_folder == 'PV':
                pv_path = list(folder.glob('*pv.txt'))
                assert len(list(pv_path)) == 1 
                (width, height) = self.get_width_and_height(pv_path[0])

                paths = (folder / img_folder).glob('*bytes')
                print("Processing images")
                for path in paths:
                    #p.apply_async(self.write_bytes_to_png, (str(path), width, height))
                    self.write_bytes_to_png(str(path), width, height)
        # p.close()
        # p.join()
...
if __name__ == '__main__':
    ...
    ci = ConvertImages()
    ci.convert_images(Path(args.recording_path))
