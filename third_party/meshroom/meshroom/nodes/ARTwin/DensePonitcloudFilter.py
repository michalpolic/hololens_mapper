    ...
    def processChunk(self, chunk):
        try:
            ...
            if chunk.node.densePointcloud.value[-4::] == ".obj":
                xyz = meshroom_io.load_obj_vertices(chunk.node.densePointcloud.value) 
                is_pointcloud_loaded = True
            if chunk.node.densePointcloud.value[-4::] == ".ply":
                xyz = meshroom_io.load_ply_vertices(chunk.node.densePointcloud.value) 
                is_pointcloud_loaded = True

            assert is_pointcloud_loaded, 'Failed to load dense pointcloud.'

            chunk.logger.info('Filtering dnese pointcloud.')
            xyz_fitered = utils_math.filter_dense_pointcloud_noise_KDtree(xyz, \
                chunk.node.neighbourDistance.value, chunk.node.minNeighbours.value)
            
            chunk.logger.info('Saving filtered pointcloud.')
            holo_io.write_pointcloud_to_file(xyz_fitered, \
                chunk.node.output.value + f"/filterd_pointcloud_{chunk.node.neighbourDistance.value}_{chunk.node.minNeighbours.value}.obj")
