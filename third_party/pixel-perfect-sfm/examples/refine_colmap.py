...
def main(dataset: Path,
    ...
    # Setup the paths
    images = dataset
    sift_sfm_dir = dataset #/ 'sfm_superpoint+superglue'
    sift_database_path = sift_sfm_dir / 'database.db'

    sfm_dir = outputs / f'sfm_{tag}'
    sfm_dir.mkdir(parents=True, exist_ok=True)
    database_path = outputs / f'database_refined_{tag}.db'
    pairs_path = sfm_dir / "pairs.txt"
    cache = outputs / f'dense_features_{tag}.h5'

    refiner = PixSfM({"dense_features": {"max_edge": 1024}})
