...
class Colmap():
    ...
    def extract_features(self, database_path, image_path, docker = False):

        self._colmap_container.command_dict("colmap feature_extractor", 
            {"database_path": ("/data" if docker else "") + database_path, 
            "image_path": ("/data" if docker else "") + image_path,
            "ImageReader.camera_model": "RADIAL",
            "ImageReader.single_camera": 1,
            "SiftExtraction.use_gpu": (0 if docker else 1)
            })

    def exhaustive_matcher(self, database_path, docker = False):
        self._colmap_container.command_dict("colmap exhaustive_matcher", 
            {"database_path": ("/data" if docker else "") + database_path,
            "SiftMatching.use_gpu": (0 if docker else 1)
            })
