...
class UtilsContainers:
    ...
    def __init__(self, container_type, image, working_dir, lib_prefix = "", logger = None):
        ...
        Input: 
            ...
            working_dir - mounted directory to /host_pwd in container
    ...
    def command(self, command):
        ...
        if self._container_type == "docker":
            command_line = f"docker run -v {self._working_dir}:/data  {self._image} {self._lib_prefix}{command}" #--gpus all
        if self._container_type == "singularity":
            command_line = f"singularity exec --nv -B {self._working_dir}:/host_pwd --pwd /host_pwd {self._image} {self._lib_prefix}{command}"
